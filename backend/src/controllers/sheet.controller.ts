...
// Obtener los encabezados de una hoja de cálculo
export const getHeadersByGoogleSheetId = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log('Google Sheet ID recibido:', googleSheetId);
    ...
    try {
      ...
      const sheetNames = sheetsResponse.data.sheets?.map(sheet => 
